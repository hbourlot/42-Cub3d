...
/*   Updated: 2025/03/24 01:20:50 by joralves         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int color)
{
	...
	ft_memset(&lines, 0, sizeof(t_dda));
	i = 0;
	lines.dx = x1 - x0;
	lines.dy = y1 - y0;
...
void	draw_player2d(t_cube3d *game)
{
	...
	// draw_square(game, game->player.x, game->player.y, TILE_SIZE, TILE_SIZE,
	// 	0xFFFF00);
	line_x = game->player.x + TILE_SIZE / 2 + game->player.pdx * 3 * TILE_SIZE;
	line_y = game->player.y + TILE_SIZE / 2 + game->player.pdy * 3 * TILE_SIZE;
	draw_line(game, game->player.x + TILE_SIZE / 2, game->player.y + TILE_SIZE
		/ 2, line_x, line_y, 0x00FF00);
