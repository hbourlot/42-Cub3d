...
/*   Updated: 2025/03/14 19:23:34 by joralves         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	draw_line(game,game->player.x * TILE_SIZE + TILE_SIZE
// /2 ,game->player.y
// * TILE_SIZE+ TILE_SIZE /2,  game->player.pdx * TILE_SIZE,
// game->player.pdy * TILE_SIZE,0xFFFF00);
// }
	...
	for (r = 0; r < 60; r++)
	{
		...
		while (dof < 8)
		{
			...
			else
			{
				...
				ry += yo;
				dof += 1;
				printf("Error: mx=%d, my=%d fuera de rango. map->witdh=%d map->height=%d\n", mx,
					my, map->width, map->height);
			}
			printf("Después: dof = %d, rx = %f, ry = %f\n", dof, rx, ry);
		}
		...
		while (dof < 8)
		{
			mx = (int)(rx) >> 6;
			my = (int)(ry) >> 6;
			mp = my * map->screen_width + mx;
			// printf("map->width %d map->height%d\n", map->width, map->height);
			if (mx >= 0 && my >= 0 && my < map->height && mx < map->width
				&& map->map_array[my][mx] == '1')
			{
				vx = rx;
				vy = ry;
		...
		// printf("Rayo %d: rx = %f, ry = %f, disT = %f\n", r, rx, ry, disT);
		draw_line(game, game->player.x * TILE_SIZE, game->player.y * TILE_SIZE, rx, ry, 0xFF0000);
		ra += DR;
		if (ra < 0)
			ra += 2 * PI;
		if (ra > 2 * PI)
