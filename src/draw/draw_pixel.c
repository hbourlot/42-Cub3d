  ...
  while (game->map->map_array && game->map->map_array[y]) {
    ...
    while (game->map->map_array[y][x]) {
      if (game->map->map_array[y][x] == '1')
        ...
        color = 0x000000;
      draw_square(game, x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE,
                  color);
      x++;
    }
    y++;
...
void	draw_player2d(t_cube3d *game) {
  ...
  draw_square(game, game->player.x - TILE_SIZE / 2,
              game->player.y - TILE_SIZE / 2, TILE_SIZE, TILE_SIZE, 0xc0c0c0);
